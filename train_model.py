...
def train(epoch):
    ...
    for (x_imu, x_s, y) in loader:
        'x_imu: (batch_size, seq_len, input_channels)'
        'x_s: (batch_size, seq_len, output_channels)'
        'y: (batch_size, seq_len, output_channels)'
        i += x_imu.size()[0]
        ...
        'what is x_s here? It is your previous model outputs, but here it is the ground truth of previous timestamp'
        y_pred = model(x_imu, x_s + noise_s)

        'seems to be a regularization for velocity vector prediction'
        'Not really, this is used to smooth the prediction of rotation matrix'
        loss_j = loss_jerk(y_pred[:, :, :-3-(n_sbps * 4)])
        'Shape of the y_pred? I think you are getting the last element as your outputs'
        'Not really, notice that -1 here is the last size, not last element'
        'So you are just checking all the y values'
...
if __name__ == '__main__':
    ...
    'load model weights if warm start'
